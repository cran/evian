...
evian <-
  function (data, bim, xcols=NULL, ycol=NULL, covariateCol=NULL, formula=NULL, robust=FALSE, model="additive", m=200, bse=5, lolim=NULL, hilim=NULL,kcutoff = c(8,32,100,1000), multiThread=1,family='gaussian',plinkCC=F)
  {
    tryCatch(
      {
        if (multiThread>1){
    ...
    if (sum(kcutoff<=1)) { stop ('K value need to be greater than 1')} #condition for k
    ...
    if (multiThread != 1){
      result=foreach(snpIndex=1:length(x_list),.combine = rbind) %dopar% {
        calculateEvianMLE(snp=x_list[snpIndex],formula_tofit=formula_tofit,model=model,data=data,bim=bim,lolim=lolim,hilim=hilim,m=m,bse=bse,k=as.vector(kcutoff),robust=robust,family=family,plinkCC=plinkCC)
      }
      parallel::stopCluster(cl)
    } else {
      result=NULL
      for (snpIndex in 1:length(x_list)){
        result=rbind(result,calculateEvianMLE(snp=x_list[snpIndex],formula_tofit=formula_tofit,model=model,data=data,bim=bim,lolim=lolim,hilim=hilim,m=m,bse=bse,k=as.vector(kcutoff),robust=robust,family=family,plinkCC=plinkCC))
      }
    }
    return(result)
  }
