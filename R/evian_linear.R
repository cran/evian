...
evian_linear <-
  function (data, bim, xcols=NULL, ycol=NULL, covariateCol=NULL, formula=NULL, robust=FALSE, model="additive", m=200, bse=5, lolim=NULL, hilim=NULL,kcutoff = c(8,32,100,1000), multiThread=1)
  {
    tryCatch(
      {
        if (multiThread>1){
    ...
    if (multiThread != 1){
        result=foreach(snpIndex=1:length(x_list),.combine = rbind) %dopar% {
          calculateLinearMLE(x_list[snpIndex],formula_tofit,model,data,bim,lolim,hilim,m,bse,as.vector(kcutoff),robust,family='gaussian')
        }
        parallel::stopCluster(cl)
    } else {
        result=NULL
        for (snpIndex in 1:length(x_list)){
          result=rbind(result,calculateLinearMLE(x_list[snpIndex],formula_tofit,model,data,bim,lolim,hilim,m,bse,as.vector(kcutoff),robust,family='gaussian'))
        }
    }
    return(result)
  }
